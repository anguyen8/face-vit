...
def removeCLSandSepForSelfAtt(x):
    h, w = x.size()
    mid = int(h/2)
    # remove rows
    x = x[1:, :]
...
def draw_img_with_flow(flows, query_img,  right_img, transformRaw, size=128, cross=True):
    # flow = 1.0 - flow
            # flow = torch.nn.functional.normalize(flow, p=1, dim=1)
    right_img_transform = transformRaw(right_img)
    image = Image.new('RGB', (3 * size, size))
    fig = plt.figure()
    axes = fig.add_subplot(111)
    caxes = axes.matshow(flows.cpu().detach().numpy(), interpolation ='nearest')
    fig.colorbar(caxes)
    fig.canvas.draw()
    flow_img = np.frombuffer(fig.canvas.tostring_rgb(), dtype=np.uint8)
    flow_img = flow_img.reshape(fig.canvas.get_width_height()[::-1] + (3,))
    # flow_img = Image.fromarray(np.uint8(cm.gist_earth(flow_img)))
    flow_img = Image.fromarray(flow_img)
    # flow_img = flow_img.resize((128,128))

    weight = flows.max(-1)[0] #flows.sum(-1)
    ...
    patch_list = []
    for index_grid in range(num):
        index_patch = torch.argmax(flows[index_grid]).item()
        # print('index_patch: ', index_patch)
        # if index_patch >= num:
        #     index_patch = index_patch - num

        row_location, col_location, _ , _ = get_patch_location(index_patch, size, level=8)
    ...
    # image.paste(flow_img, ((0, 0)))
    image.paste(query_img, ((0, 0)))
    image.paste(grid_img, (1*size, 0))
    image.paste(right_img, (2*size, 0))
    
    return image, flow_img
...
if __name__ == '__main__':
    ...
    layer_id = 0
    head_id = 0
    caption = 'diff' #'mask' #'identical' #'glass' #
    ...
    for d in range(depth_vit):
        for hid in range(heads):
            ...
            flow_cross =  removeCLSandSep(attns[d][0,hid, :, :])
            flow_self = removeCLSandSepForSelfAtt(attns[d][0,hid, :, :])

            cross_img, cross_mat = draw_img_with_flow(flow_cross, query_img, right_img, transformRaw, size=size)
            sefl_img, self_mat = draw_img_with_flow(flow_self, query_img, query_img, transformRaw, size=size, cross=False)

            
            cross_imgname = 'flows/cross_flow_face_{}_{}_vs_{}_depth_{}_did_{}_head_{}.jpg'.format(caption, name_1, name_2, depth_vit, d, hid)
            cross_mat_img = 'flows/cross_flow_face_mat_{}_{}_vs_{}_depth_{}_did_{}_head_{}.jpg'.format(caption, name_1, name_2, depth_vit, d, hid)
            self_imgname = 'flows/self_flow_face_{}_{}_vs_{}_depth_{}_did_{}_head_{}.jpg'.format(caption, name_1, name_2, depth_vit, d, hid)
            self_mat_img = 'flows/self_flow_face_mat_{}_{}_vs_{}_depth_{}_did_{}_head_{}.jpg'.format(caption, name_1, name_2, depth_vit, d, hid)
        
            print('cross img: {}'.format(cross_imgname))
            print('cross mat flow img: {}'.format(cross_mat_img))
            print('self img: {}'.format(self_imgname))
            print('self mat flow img: {}'.format(self_mat_img))
            cross_img.save(cross_imgname)
            cross_mat.save(cross_mat_img)
            sefl_img.save(self_imgname)
            self_mat.save(self_mat_img)
