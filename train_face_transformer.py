...
from util.utils import get_val_data, perform_val, perform_val_color_images, get_time, buffer_val, AverageMeter, train_accuracy
...
use_img_color = False
if use_wandb:
    wandb.init(project="ViT-P8S8_ms1m_cosface_s1-depth-{}-heads-{}".format(depth_vit, num_heads), entity="your name")
        ...
        for inputs, labels in iter(trainloader):
            ...
            if ((batch + 1) % VER_FREQ == 0) and batch != 0: #perform validation & save checkpoints (buffer for visualization)
                ...
                for ver in vers:
                    if use_img_color:
                        name = ver
                        accuracy, std, xnorm, best_threshold, roc_curve = perform_val_color_images(MULTI_GPU, DEVICE, EMBEDDING_SIZE, BATCH_SIZE, BACKBONE, grayscale=False, size=112, use_scale=False)  
                    else:
